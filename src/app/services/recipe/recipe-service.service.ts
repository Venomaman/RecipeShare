...
export class RecipeServiceService {
  ...
  updateRecipes(recipe:any): Observable<any> {
    ...
    return this.http.put(`${this.baseUrl}/api/recipe/${recipe.id}`,recipe,{ headers }).pipe(
      tap((updatedRecipe:any) => {
        ...
        item.id===updatedRecipe.id? updateRecipes:item)
  ...
  likeRecipes(id:any): Observable<any> {
    const headers = this.getHeaders();
    return this.http.put(`${this.baseUrl}/api/recipe/${id}`,{ headers }).pipe(
      tap((updatedRecipe:any) => {
        const currentState = this.recipeSubject.value;
        const updateRecipes = currentState.recipes.map((item:any)=>
        item.id===updatedRecipe.id? updateRecipes:item)
  ...
  deleteRecipes(id:any): Observable<any> {
    ...
    return this.http.delete(`${this.baseUrl}/api/recipe/${id}`,{ headers }).pipe(
      tap((deletedRecipe:any) => {
        const currentState = this.recipeSubject.value;
        const updateRecipes = currentState.recipes.filter((item:any)=>
        item.id!==id)
        this.recipeSubject.next({ ...currentState, 
          recipes:updateRecipes });
      })
